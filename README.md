...
`data-processing.py` extracts the positive and negative sets for training, and computes the number of variants within each context window. The output is used as input for `feature-annotation.py`, which annotates each sample with various curated genomic features, such as conservation scores, repeat content, and gene features. This produces an annotated, balanced dataset of SV and background regions, which is then used for subsequent analyses.     

### Model analyses
`model-training-predictions.ipynb` trains and tests each model (CNN, random forest, logistic regression, and ensemble) using the generated annotated dataset, and runs further analysis to compute feature importances of genomic annotations on model predictions and representational analyses on the CNN.       
